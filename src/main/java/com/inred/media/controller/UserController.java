...
public class UserController extends BaseController<User> {
	...
	public ModelAndView createOrUpdateUserToFind(User webUser, HttpSession session) {
		...
		webUser.setType(webUser.getType());
	...
	public ModelAndView findlist(@RequestParam(value = "pid", defaultValue = "0") String pid,
		...
		if (user.getType().equals(UserType.TEACHERUSER)) {
			return modelAndView;
		}

		// 分页查询数据
		Query query = new Query();
		if (user.getType().equals(UserType.SCHOOLMANAGER)) {
			query.addCriteria(Criteria.where("type").in(Arrays.asList(UserType.SCHOOLMANAGER, UserType.TEACHERUSER)));
		}

		Pagination<User> pagination = userServiceI.findPaginationByQuery(query, pageNo, pageSize, User.class);
//		Pagination<User> pagination = userServiceI.findPaginationByQuery(craeteQueryWhere("schoolId",user.getSchoolId()), pageNo, pageSize, User.class);
	...
	public ModelAndView personalCenter(HttpSession session) {
		...
		// 加载有几条个人通知信息
		Integer notifcation = this.schoolNotificationI
				.findCountByQuery(super.craeteQueryWhere("schoolId", user.getSchoolId()), SchoolNotification.class);
	...
	public ModelAndView index() {
		...
		// 加载最新上传的资源10条
		Pagination<Resources> listNewResources = this.resourcesServiceI.findPaginationByQuery(new Query(), 1, 10,
				Resources.class);

		// 加载学校所有科目信息
		List<Subject> listSub = this.subjectServiceI.find(new Query(), Subject.class);
		...
		// 添加科目集合信息
		modelAndView.addObject("listSub", listSub);
	 ...
	 * @return
	 */
	@RequestMapping("/loginTeacherinfoSOS")
	public ModelAndView loginInfoSOS(String username, HttpServletRequest request) {
		ModelAndView modelAndView = new ModelAndView();
		...
		try {
			...
			String md5Pass = FileHashUtil.stringMD5("cored");

//			User user = userServiceI.userLogin("cored", "cored");
			User user = userServiceI.findOneByQuery(super.craeteQueryWhere("username", "shining"), User.class);

			if (user != null) {
				user.setUsername(username);// 替换用户名
				user.setName(username);

				request.getSession().setAttribute(Constant.LOGIN_USR_AUTH, user);
	...
	public ModelAndView loginInfo(String username, HttpServletRequest request, String schoolIds, String password) {
		ModelAndView modelAndView = new ModelAndView();
		modelAndView.setViewName("loginForTeacher");
		try {
			if (StringUtils.isEmpty(username)) {
...
//			}
	...
	public ModelAndView personalUpdate(String id, String username, String name, String remark, String resourcesRole,
			HttpSession session) {
	...
	public ModelAndView personalUpdatePassword(HttpSession session, String id, String password, String oldPassword) {
		ModelAndView modelAndView = new ModelAndView();
