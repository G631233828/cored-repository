...
import com.inred.media.XSS.XSSRequestWrapper;
...
public class ResourcesCenterController extends BaseController<Resources> {
	...
	public ModelAndView resourceUploadViews(HttpServletRequest request,
			QueryResourcesBean queryResourceBean,String resourcesType) {
		...
		mav.addObject("url", XSSRequestWrapper.stripXSS(urlEnc(request.getParameterMap())));
	...
	@RequestMapping("/toWmvPage")
	public ModelAndView toWmvPage(){
		
		ModelAndView mav = new ModelAndView();
		mav.setViewName("resources/video");
		return mav;
	}
